...
- the symbolic engine remains buildable and testable
- the SDK path now has explicit SDK and IR boundaries
- the SDK implementation is still ahead of some symbolic-engine boundary cleanup work

## Recommended Developer Workflow

1. Add new implementation under SDK targets first.
2. Reuse symbolic-engine code only after isolating the dependency and documenting why it is safe.
...
- Can it be expressed without exposing symbolic `Expr` internals?
