...
namespace aleph3 {
    ...
    std::vector<ExprPtr> factor_univariate_exact_polynomial(
        ...
        std::vector<ExprPtr> factors;
        if (poly.degree() <= 1) {
            factors.push_back(exact_polynomial_to_expr_impl(poly));
            return factors;
        }
        ...
        if (remainder_poly.degree() == 1) {
            linear_factors.emplace_back(
                ...
                exact_polynomial_to_expr_impl(remainder_poly));
        } else if (!is_exact_constant_one(remainder_poly)) {
            factors.push_back(exact_polynomial_to_expr_impl(remainder_poly));
        }
    ...
    ExprPtr factor_exact_polynomial(
        const ExactPolynomial& poly,
        const std::vector<std::string>& variables) {
        if (poly.is_zero()) return exact_polynomial_to_expr_impl(poly);
        ...
        if (!content.coefficient.is_one() || !content.monomial.empty()) {
            ...
            if (!content.monomial.empty()) {
                content_factors.push_back(
                    exact_polynomial_to_expr_impl(monomial_to_exact_polynomial(content.monomial)));
            }
        ...
        if (remaining_variables.size() == 1
            ...
            auto univariate_factors = factor_univariate_exact_polynomial(
                content.primitive,
                remaining_variables.front());
            ...
            factors.push_back(exact_polynomial_to_expr_impl(content.primitive));
        }

        if (factors.empty()) return exact_polynomial_to_expr_impl(content.primitive);
