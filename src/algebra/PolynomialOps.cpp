...
PolynomialContent split_content(const Polynomial& poly) {
    ...
    for (const auto& [mono, coeff] : poly.terms) {
        ...
        if (!is_near_integer(coeff)) {
            ...
            coefficient_gcd =
                coefficient_gcd == 0 ? abs_coeff : std::gcd(coefficient_gcd, abs_coeff);
        }
    }

    if (first_term) return content;

    if (!all_integer_coefficients || coefficient_gcd == 0) coefficient_gcd = 1;
    ...
    content.coefficient =
        static_cast<double>(coefficient_gcd) * (leading_negative ? -1.0 : 1.0);
    ...
    if (leading_it != primitive.terms.end() && leading_it->second < 0.0) {
        for (auto& [_, coeff] : primitive.terms) coeff = -coeff;
        primitive.normalize();
...
std::vector<int64_t> univariate_coefficients(
    ...
    for (const auto& [mono, coeff] : poly.terms) {
        ...
        int exponent = 0;
        auto it = mono.find(var);
        if (it != mono.end()) exponent = it->second;
...
Polynomial coefficients_to_polynomial(
    ...
    for (size_t i = 0; i < coefficients.size(); ++i) {
        ...
        Monomial mono;
        if (exponent > 0) mono[var] = exponent;
...
ExprPtr polynomial_linear_expr(
    ...
    if (is_near_zero(variable_coefficient)) return make_expr<Number>(constant_term);
...
std::vector<ExprPtr> factor_univariate_polynomial(
    ...
    if (remainder.degree() == 1) {
        ...
        if (it != remainder.terms.end()) constant_term = it->second;
    ...
    std::sort(
        ...
        [](const auto& left, const auto& right) {
            ...
            if (!is_near_zero(left_value - right_value)) return left_value > right_value;
...
Polynomial expr_to_polynomial(
    ...
    const auto make_monomial = [&](const std::map<std::string, int>& exps) -> Monomial {
        Monomial m;
        for (const auto& v : variables) {
            auto it = exps.find(v);
            if (it != exps.end() && it->second != 0) m[v] = it->second;
        }
        return m;
    };
    ...
    std::function<Polynomial(const ExprPtr&)> recur = [&](const ExprPtr& e) -> Polynomial {
        ...
        if (auto minus = std::get_if<FunctionCall>(&(*e)); minus && minus->head == "Minus") {
            if (minus->args.size() == 2) return recur(minus->args[0]) - recur(minus->args[1]);
        }
        if (auto negate = std::get_if<FunctionCall>(&(*e));
            negate && negate->head == "Negate") {
            if (negate->args.size() == 1) return Polynomial(0.0) - recur(negate->args[0]);
        }
...
ExprPtr polynomial_to_expr(const Polynomial& poly) {
    ...
    for (const auto& [mono, coeff] : terms_in_order) {
        ...
        std::vector<ExprPtr> factors;
        if (coeff != 1.0) factors.push_back(make_expr<Number>(coeff));
...
ExprPtr factor_approximate_polynomial(const Polynomial& poly) {
    if (is_constant_zero(poly)) return polynomial_to_expr(poly);
    ...
    const auto remaining_variables = infer_polynomial_variables(remaining);
    ...
    if (factors.empty()) return polynomial_to_expr(remaining);
